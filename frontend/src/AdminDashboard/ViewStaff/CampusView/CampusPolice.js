...
const navigate=useNavigate();
 useEffect(()=>{
  ajax('/api/campus_users/campus_polices','GET',user.jwt).
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ajax(`/api/campus_users/${staffId}`, 'delete', user.jwt).then((data)=>{
  ...
  return (
    <div>
          <ViewStaffHeader/>
          <h1 className={styles.registrarTitle}>List of Campus Polices</h1>
          {datas ? (
                        ...
                        datas.map((data) => (
                            <Card key={data.id} style={{ width: "30rem", height: "30rem" }}  className={`${styles.card}`}>
                                <Card.Body className="d-flex flex-column justify-content-around">
                                    <Card.Title className={`${styles.card_title}`}>{data.campus.name}</Card.Title>
